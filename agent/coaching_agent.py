"""
coaching_agent.py — Main Coaching Agent
Orchestrates: Context Receiver → RAG Retrieval → LLM Generation → Polish → Output
"""
...
class CoachingAgent:
    """
    Coaching Agent: generates personalized physiotherapy coaching feedback.
    
    Pipeline:
    PatientContext → RAG retrieval → gemma3:4b generation → Polish → CoachingOutput
    """
    ...
    def __init__(
        ...
        if self.verbose:
            print(f"CoachingAgent ready — model: {ollama_model}, retrieval_k: {retrieval_k}")

    # ── Main entry point ─────────────────────────────────────────────────────

    def generate_coaching(self, context: PatientContext) -> CoachingOutput:
        """
...
def run_demo():
    ...
    # Create agent
    agent = CoachingAgent(knowledge_base=kb, verbose=True)

    # Run three scenarios
    for scenario in ["knee", "shoulder", "acl"]:
        context = make_sample_context(scenario)
        output = agent.generate_coaching(context)
